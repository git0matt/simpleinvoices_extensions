...
foreach($extensions as $row) {
    ...
    $xml .= "]]></cell>";
	$xml .= "<cell><![CDATA[" . $row['id'];
	if (file_exists("./extensions/{$row['name']}/README"))
	{
		$xml .= '<a class="cluetip2" title="';
		//if (isset($LANG['help_for']))
		//	$xml .= $LANG['help_for'];
		//else
		//	$xml .= 'HElp for';
		//$xml .= ' `'. $row['name']. '`|';
		//$xml .= '<pre>';
		$xml .= file_get_contents("./extensions/{$row['name']}/README");
		//$xml .= '</pre>';
		$xml .= '" href="#" rel="#readme_'. $row['id']. '">
				<img src="images/common/help-small.png" alt="help-small">
			</a>
		<div id="readme_'. $row['id']. '" style="display: none;">';//>';//
		$xml .= '</div>';
	}
	$xml .= "]]></cell>";
